function [IMU,paramsout]= DenoisingAlgorithm(I1,I7,paramsin)
...
% conditioned doubly sparse parameters
lambda = paramsin.lambda;
homotopy_steps = paramsin.homotopy_steps;
debias_start = paramsin.debias_start;
clipping_eps = paramsin.clipping_eps;

% params for Doubly Sparse Bresler
B = paramsin.B;
T1 = paramsin.T1;

if strcmp(paramsin.method, 'BreslerDoublySparse') || strcmp(paramsin.method, 'DoublySparseConditionedTL')
    Wlast = B;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    W = ClosedFormBreslerDoublySparse(Wbresler,YH,YH,numiterr,l2,l3,full_sty,STY,STY);  %Transform Learning with log-determinant+Frobenius norm regularizer
    condW = cond(W);
    froW = norm(W,'fro');
    Wbresler = W;
    %Transform Learning Step
    if(strcmp(paramsin.method, 'DCT'))
        W = W0;
    elseif(strcmp(paramsin.method, 'Bresler'))
        W = ClosedFormBreslerDoublySparse(Wlast,YH,YH,numiterr,l2,l3,full_sty,STY,STY);
    elseif(strcmp(paramsin.method, 'ConditionedTransformLearning'))
    	W = ConditionedTransformLearning(Wlast,YH,YH,numiterr,STY,STY,condW,froW);
    elseif(strcmp(paramsin.method, 'BreslerDoublySparse'))
    	W = ClosedFormBreslerDoublySparse(Wlast,YH2,YH2,numiterr,l2,l3,T1,STY,STY);
    elseif(strcmp(paramsin.method, 'DoublySparseConditionedTL'))
    	W = DoublySparseConditionedTL(Wlast,YH2,YH2,numiterr,STY,STY,condW,froW,lambda, homotopy_steps, debias_start, clipping_eps);
        total = numel(W);           
        curr_sty = nnz(W(:) ~= 0);  
        sty_pct = 100 * curr_sty / total;  
        fprintf('Method: %s, Transform sparsity: %.2f%%\n', paramsin.method, sty_pct);
    end
