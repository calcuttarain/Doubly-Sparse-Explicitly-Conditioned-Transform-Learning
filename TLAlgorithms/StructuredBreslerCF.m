function [B,XB,error]= StructuredBreslerCF(B, Y, Y_test, numiter, l2, l3, T1, STY, STY_te);
cbb = 0;
...
%     W = sqrt(n)*W/norm(W, 'fro');
    ...
    error.m2.tr(i) = error.m1.tr(i) / norm(BY, 'fro');
    error.m1.te(i) = norm(X_test - BY_test, 'fro');
    error.m2.te(i) = error.m1.te(i) / norm(BY_test, 'fro');
end
XB(:,ix)=X;
